...
<tr><td Height="35"><font size="2">${status.index+1}</font></td>
	...
	<td><font size="2"><a href="javascript:void(0)" onclick="album('${hm[0].offense.album.albumID}')">${hm[0].offense.album.name}</a></font></td>
	<td><font size="2"><a href="javascript:void(0)" onclick="creator('${hm[0].offense.album.creator.memberId}')">${hm[0].offense.album.creator.userName}</a></font></td>
	<td><font size="2"><a href="javascript:void(0)" onclick="offenseReason('${hm[0].offense.album.albumID}')">檢舉原因</a></font></td>
	<td><font size="2">${hm[0].startDate}&nbsp已隱藏</font></td>
	<td><input type="submit" value="取消隱藏" onclick="noHidden('${hm[0].offense.album.albumID}')"/></td>
	<td><input type="submit" value="確定隱藏" onclick="yesHidden('${hm[0].offense.album.albumID}')"/></td></tr>
	</c:forEach>
</table>
</form>
</body>
<script>
function query(){
     document.fm.action="${pageContext.request.contextPath}/queryHiddenAlbum.do";
     document.fm.submit();
}
function noHidden(Id){
    document.fm.action="${pageContext.request.contextPath}/noHidden.do?ID="+Id;
    document.fm.submit();
}
function yesHidden(Id){
    document.fm.action="${pageContext.request.contextPath}/yesHidden.do?ID="+Id;
    document.fm.submit();
}
