...
<form name="fm" method="post">	
	...
	處理狀態 :<select name="handleStatus" id="handleStatus">
				<option></option>
...
<tr><td Width="100" Height="35" valign="top"><font size="2">提問時間</font></td>
	...
	<fmt:parseDate var="dateObj" value="${hm.questionDate}" type="DATE" pattern="yyyyMMddHHmmss"/> 
	...
	<td><font size="2">${hm.email}</font></td>
	<td><font size="2">${hm.tel}</font></td>
	<td><font size="2">
	...
	<td><font size="2"><c:if test="${hm.handleStatus==1}">未處理</c:if><c:if test="${hm.handleStatus==2}">已處理</c:if></font></td>
	<td><input type="button" value="查看" onclick="queryQuestionDetail(${admin},'${hm.id}')"></td>
	<td>
	<c:choose>
	<c:when test="${hm.noteContent != null}"><font size="2">V</font></c:when>
